...
Live digital services, from train-delay trackers to home-energy dashboards, depend on streams of fresh data delivered through public \glspl{API}. Behind the scenes, two technical decisions dominate how fast and cheaply those streams reach end-users: \emph{(i)} whether the back-end pulls updates at fixed intervals (polling) or reacts to push events (event driven), and \emph{(ii)} which kind of database stores the incoming records; relational, specialised time-series, or cloud \glspl{NoSQL}. Prior work shows that each option shifts the balance between latency, throughput, and resource cost \cite{Trindade2021EDAImpact,Heldt2021SciTS}.  
...
The remainder of the chapter states the background (Section 1.1), the problem (Section 1.2), defines the purpose and goals (Sections 1.3–1.4), sketches the method (Section 1.5), lists delimitations (Section 1.6), and outlines the structure of the thesis (Section 1.7).
