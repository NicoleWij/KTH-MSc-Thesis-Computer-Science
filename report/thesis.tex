...
Live digital services, from train-delay trackers to home-energy dashboards, depend on streams of fresh data delivered through public \glspl{API}. Behind the scenes, two technical decisions dominate how quickly those streams reach end-users and how much computing effort they consume: \emph{(i)} whether the back-end pulls updates at fixed intervals (polling) or reacts to push events (event driven), and \emph{(ii)} which type of database stores the incoming records; relational, specialised time-series, or cloud NoSQL. Prior work shows that each option shifts the balance between latency, throughput, and resource utilisation \cite{Trindade2021EDAImpact,Heldt2021SciTS}.
...
Many online services rely on a steady flow of information coming from other websites.
To keep that information useful, the back-end has to fetch it fast enough so it isn’t already out of date when people see it. In a cloud system, two main design choices largely determine how fresh the data stays:

Live online services increasingly draw on a steady stream of data from other platforms. The back-end must pull this information from public or partner APIs fast enough that it is still relevant when users receive it. In a cloud environment, two key architectural choices—how updates are fetched and where they are stored—largely decide how fresh the data stays:

Live digital services increasingly rely on a continuous integration of external data streams.  
Any application that depends on real-time updates must ingest public or partner \glspl{API} quickly enough for the information to remain actionable by the time it reaches end-users. In a cloud back-end, two architectural decisions dominate that end-to-end freshness:
...
Prior empirical work shows that each choice shifts the balance between latency, throughput, and scalability.  
For example, Trindade \etal\ quantify how event-driven micro-services trim tail-latency compared with CRON-based pollers under the same load profile\cite{Trindade2021EDAImpact}, while Heldt \etal\ observe that purpose-built time-series databases sustain 2-3 times higher range-query throughput than row-oriented stores on identical hardware\cite{Heldt2021SciTS}. Yet those studies are evaluated in isolation, either ingestion or storage, and seldom under the hard rate limits that public APIs impose.

\subsection{Why ingestion strategy still matters}  
Polling remains attractive because it is simple, firewall-friendly, and easy to parallelise, but it also wastes network budget when nothing has changed.  
Event-driven designs—built on webhooks, message queues, or CDC streams—push updates only when required, reducing duplicate traffic and smoothing burst loads\cite{Silva2022EDAMicroservices}.  
Energy-constrained IoT deployments show power savings above 30 \% when moving from timer-based fetches to publish/subscribe patterns\cite{Makarovi2022EnergyEfficientIoT}.  
The open question is whether those latency and efficiency gains persist once the pipeline is coupled to different classes of databases.

\subsection{Storage engines for real-time feeds}  
Relational systems such as PostgreSQL offer mature SQL semantics and transactional guarantees, but their B-tree layouts become write-amplified under high-frequency inserts.  
Time-series engines—e.g.\ TimescaleDB, InfluxDB, or Amazon Timestream—append data in chunked, column-oriented blocks and co-locate recent chunks in memory caches, yielding order-of-magnitude faster window queries\cite{Grzesik2020EdgeIoTBenchmark, Vergara2021PerformanceTSDB}.  
Cloud NoSQL services like DynamoDB trade single-query latency for \say{dial-a-throughput} scalability; Johansson’s evaluation of high-frequency sensor streams in AWS shows linear write scaling well past ten million rows per minute, at the cost of ~2 × longer median read latency compared with RDS-PostgreSQL\cite{Johansson2022AWSCloudData}.  

\subsection{Need for a holistic comparison}  
Although benchmarks exist for isolated components, no prior study controls both ingestion style and storage engine under the same API-rate constraints.  
Consequently, engineers still rely on tribal knowledge when choosing architectures for real-time, API-dependent workloads, and researchers lack a reproducible baseline for follow-up optimisation studies.  

To fill this gap, the present thesis will (i) implement matched polling and event-driven pipelines, (ii) deploy them with PostgreSQL, Timestream, and DynamoDB, and (iii) measure end-to-end update latency, query performance, and resource usage. 

The results aim to give practitioners a data-driven recipe for picking the right stack and to provide the research community with a public, reproducible benchmark.
